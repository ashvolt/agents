...
AI = ROOT / "ai_cache"
SOURCE = AI / "diffusiondb"
INDEX = AI / "diffusiondb_index.jsonl"
...
def cut_out(img: Image.Image) -> tuple[Image.Image, bool]:
...
def _sources(exclude: list[Path] | None) -> list[dict]:
    if not INDEX.exists():
        raise SystemExit("no AI artwork. Run: python -m capstone.data.fetch_ai_art")
    used = {
    ...
    return [
        r
        for r in rows
        if (SOURCE / r["image_name"]).exists() and f"diffusiondb/{r['image_name']}" not in used
    ]
...
def build(
    ...
    `exclude` manifest used it."""
    sources = _sources(exclude)
    if len(sources) < n:
    ...
    rows = []
    for plan, source in zip(plans, sources[:n], strict=True):
        art, cutout = cut_out(Image.open(SOURCE / source["image_name"]))
        img, dpi, achieved_de = render_real(plan, art, plan.seed)
        path = save_case(img, dpi, plan, out_dir)
        label = GoldLabel(
            case_id=plan.case_id,
            perturbations=drawn_perturbations(plan, achieved_de),
            split=plan.split,
        )
        rows.append(
            {
                ...
                "style": "diffusiondb",
                "art": f"diffusiondb/{source['image_name']}",
                "cutout": cutout,
